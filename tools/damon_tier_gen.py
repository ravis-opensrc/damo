...
# Headroom for the cold case even where the derived floor is small: four times a
# floor of 1000 leaves a kdamond splitting into 4000 regions, which the merge
# step reclaims as fast as the split creates them.
_MAX_NR_REGIONS_COLD_FLOOR = 20000
...
def build_hot_context(sources, ops, near_node=0, far_node=1,
    ...
    if max_nr_regions is None:
        ...
        max_nr_regions = (max(_MAX_NR_REGIONS_COLD_FLOOR, min_nr_regions * 4)
                          if cold_demote else 1000)
...
def main():
    ...
    parser.add_argument('--max_nr_regions', type=int, default=None,
                        help='max_nr_regions (default 1000; with '
                             '--cold_demote, the larger of %d and 4x the min)'
                             % _MAX_NR_REGIONS_COLD_FLOOR)
