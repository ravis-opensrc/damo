    ...
    if prep.prep_action == _damon.prep_action_perf_event:
        for attr in _damon._PERF_EVENT_ATTRS:
            ...
            if not os.path.isfile(fpath):
                # The caller set this one, so the kernel not taking it means
                # the probe would be armed on some other event: a config the
                # kernel supplies rather than the one asked for, reporting
                # hotness for something else with nothing to say so.  A value
                # the caller left unset was skipped above; this is the case
                # where the distinction has to be kept.
                return ('kernel does not support perf event attribute %s '
                        '(expected %s)' % (attr, fpath))
...
def write_pause(pause_file, pause_val):
    if not os.path.isfile(pause_file):
        return None
    return _damo_fs.write_file(pause_file, 'Y' if pause_val else 'N')
...
def write_context_dir(dir_path, context):
    ...
    err = write_pause(os.path.join(dir_path, 'pause'), context.pause)
    if err is not None:
        return err
...
def files_content_to_context(files_content):
    ...
    if 'pause' in files_content:
        pause = files_content['pause'].strip()
    else:
        pause = False
