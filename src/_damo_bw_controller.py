...
class BwController:
    ...
    def step(self, readings):
        ...
        # magnitude comparisons symmetric about zero.
        ...
        if cur_bw < self.bw_cutoff:
            ...
            bw_int_ratio = abs((bw_change * 100) // interleave_change) if interleave_change != 0 else 0
            do_throttle = (bw_int_ratio < THROTTLE_THRESHOLD and
                           abs(last_step) > MIN_THROTTLE_STEP)
            if do_throttle:
                throttle_step = bw_int_ratio * last_step // 100
