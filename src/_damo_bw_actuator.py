...
# What a convergence wait observed.  Several states rather than one bit, because
# the caller's next reading means something different in each: at the target it
# describes the new operating point, where the wait ran out or stopped
# progressing it describes a system that had not stopped moving, and where
# nothing could be observed it describes nothing at all.
CONV_REACHED = 'reached'
CONV_STALLED = 'stalled'
CONV_DEADLINE = 'deadline'
# No signal to decide on: the tracepoint the wait reads was not available, so
# the fallback saw only whether anything had been applied at all.  Distinct from
# the other two, which are verdicts on a system that was being watched.
CONV_UNOBSERVED = 'unobserved'
...
class GoalActuator(Actuator):
    ...
    def nr_applied(self):
        # None rather than a number when the stats could not be re-read, so that
        # a reading which does not exist cannot be compared with the one before
        # it and found equal.  Without the refresh the sum comes off the
        # snapshot taken at construction, so it never moves and every window
        # reads as quiet.
        if not self._refresh_stats():
            return None
        total = 0
    ...
    def converged(self, target_dram_bp=None, tolerance_bp=500,
        ...
        if own_reader:
            ...
            try:
                ...
                # The outcome follows the answer rather than preceding it: a
                # fallback that found nothing applied did not reach anything,
                # and recording otherwise makes the loop log a wait it did not
                # have as one it did.
                nr = self.nr_applied()
                reached = nr is not None and nr > 0
                self.outcome = CONV_REACHED if reached else CONV_UNOBSERVED
                return reached
