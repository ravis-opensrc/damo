...
class TimeWindowedAverage:
    ...
    def is_stable(self, stable_window_ms, now=None):
        """True only if the window spans at least stable_window_ms.

        Prevents premature convergence when window just started filling.
        """
        if not self._samples:
            return False
        if now is None:
        ...
        return (now - oldest) * 1000 >= stable_window_ms
